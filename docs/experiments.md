...
토킹헤드 아바타 실험은 두 단계로 나눈다. `static` backend는 ffmpeg로 정지 얼굴과 음성을 합성해 전체 배선을 검증하는 폴백이고, `wav2lip` backend는 외부 Wav2Lip repo/checkpoint를 호출해 실제 입모양 동기화 mp4를 만든다.
...
아직 Wav2Lip 실측값은 환경 준비 후 채운다. 현재 목표는 제품 품질 아바타가 아니라, 음성 합성 결과를 실제 lip-sync 모델까지 연결하고 병목을 수치로 분리하는 것이다.
