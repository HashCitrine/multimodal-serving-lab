...
- 서빙 의미: 메모리 반감 = **GPU당 모델/replica 밀도 2×**. Phase 1의 '복제로 처리량↑' 레버와 직결
...
### 정리 (Phase 1 + Phase 2 종합)
- **최적화는 도움 되는 '축'이 제각각이다**: Phase 1 복제 = *throughput*, Phase 2 int8 = *memory*.
  배칭은 CPU per-utterance TTS에서 0×, int8은 CPU에서 속도 0× but 메모리 −50%.
- 즉 **"이 기법이 이 모델·이 하드웨어에서 어느 축에 듣는지"를 가정하지 말고 측정**해야 한다 — 이게
  서빙/최적화 엔지니어의 핵심 판단. (Phase 3 LLM에서 대비되는 결론을 확인 — 아래.)
...
### 정리 (Phase 1·2·3 종합) — "측정 없이는 모른다"의 3대 대조
| | 레버 | 결과 | 이유 |
|--|--|--|--|
| Phase 1 TTS (CPU) | 동적 배칭 | throughput **0×** → 복제로 2× | per-utterance·연산 바운드, 배치 forward 없음 |
| Phase 2 STT (CPU) | int8 양자화 | 속도 **0×**(약간 ↓), 메모리 **−50%** | 연산 바운드, float32 GEMM 이미 최적 |
| Phase 3 LLM (Metal) | 양자화 + 배칭 | 속도 **2.1×** + throughput **1.45×** | decode 메모리 대역폭 바운드 + 배치 forward 가능 |
