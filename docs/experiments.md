...
cascade 기준선 측정(로컬 MPS, base.en + llama3.2:1b-q4 + Piper, 1턴): STT 411 ms / LLM TTFT 1577 ms / TTS 390 ms → TTFA·E2E 약 2.4 s, RTF 0.73. 캐스케이드는 비스트리밍이라 첫 오디오가 TTS 종료 후 나와 TTFA=E2E다. CSM 으로 TTS 를 바꾸면 같은 STT/LLM 위에서 운율·표현력이 올라가는 대신 합성 비용이 커진다(Piper 대비) — 같은 `_synthesize` 교체점에서 직접 비교한다.

이 비교의 의의는 "지연 vs 관찰가능성 vs 표현력"을 같은 측정 축에서 본다는 점이다. 캐스케이드는 어디서 틀렸는지(STT 오인식/LLM 응답/TTS) 단계별로 짚을 수 있고, CSM 은 그 경계를 유지한 채 TTS 품질을 올리며, Moshi 는 경계 자체를 없애 full-duplex 자연스러움을 얻는다.
